...
#  zukgit
# gudongzengjianchi_zukgit_website  =   https://tushare.pro/document/2?doc_id=175
createexcel('股东增减持.xlsx')
stk_holdertrade_book = load_workbook('C:\\Users\\zhuzj5\\Desktop\\zbin\\J0_Data\\股东增减持.xlsx')
stk_holdertrade_excel_writer = pd.ExcelWriter('C:\\Users\\zhuzj5\\Desktop\\zbin\\J0_Data\\股东增减持.xlsx', engine='openpyxl')
stk_holdertrade_excel_writer.book = stk_holdertrade_book
stk_holdertrade_excel_writer.sheets = dict((ws.title, ws) for ws in stk_holdertrade_book.worksheets)
stk_holdertrade_data_List = pd.DataFrame()
stk_holdertrade_item_0_tscode_list = list() 
stk_holdertrade_item_0 = pro.stk_holdertrade(start_date='20210101',end_date='20211130', fields='ts_code,ann_date,holder_name,holder_type,in_de,change_vol,change_ratio,after_share,after_ratio,avg_price,total_share,begin_date,close_date')
print("股东增减持  stk_holdertrade_item_0 返回数据 row 行数 = "+str(stk_holdertrade_item_0.shape[0]))
for ts_code_sh in stk_holdertrade_item_0['ts_code']:
    stock_name = tscode_name_dict.get(ts_code_sh)
    if stock_name is None:
        stock_name = 'null'
    stk_holdertrade_item_0_tscode_list.append(stock_name)
stk_holdertrade_item_0_addname_dataframe=pd.DataFrame()
stk_holdertrade_item_0_addname_dataframe['cname'] = stk_holdertrade_item_0_tscode_list
for table_name in stk_holdertrade_item_0.columns.values.tolist():
    stk_holdertrade_item_0_addname_dataframe[table_name] = stk_holdertrade_item_0[table_name]
stk_holdertrade_data_List = stk_holdertrade_data_List.append(stk_holdertrade_item_0_addname_dataframe)


print("stk_holdertrade_data_List.__len__() = "+str(stk_holdertrade_data_List.__len__()))
stk_holdertrade_data_List.to_excel(stk_holdertrade_excel_writer,'股东增减持',index=False)
stk_holdertrade_excel_writer.save()
