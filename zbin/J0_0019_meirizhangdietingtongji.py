...
#  zukgit
# meirizhangdietingtongji_zukgit_website  =   https://tushare.pro/document/2?doc_id=198
createexcel('最近月涨跌停统计.xlsx')
limit_list_book = load_workbook('C:\\Users\\zhuzj5\\Desktop\\zbin\\J0_Data\\最近月涨跌停统计.xlsx')
limit_list_excel_writer = pd.ExcelWriter('C:\\Users\\zhuzj5\\Desktop\\zbin\\J0_Data\\最近月涨跌停统计.xlsx', engine='openpyxl')
limit_list_excel_writer.book = limit_list_book
limit_list_excel_writer.sheets = dict((ws.title, ws) for ws in limit_list_book.worksheets)
limit_list_data_List = pd.DataFrame()
limit_list_item_0_tscode_list = list() 
limit_list_item_0 = pro.limit_list(limit_type='U',end_date='20211130', fields='trade_date,ts_code,name,close,pct_chg,amp,fc_ratio,fl_ratio,fd_amount,first_time,last_time,open_times,strth,limit')
print("最近月涨跌停统计  limit_list_item_0 返回数据 row 行数 = "+str(limit_list_item_0.shape[0]))
for ts_code_sh in limit_list_item_0['ts_code']:
    stock_name = tscode_name_dict.get(ts_code_sh)
    if stock_name is None:
        stock_name = 'null'
    limit_list_item_0_tscode_list.append(stock_name)
limit_list_item_0_addname_dataframe=pd.DataFrame()
limit_list_item_0_addname_dataframe['cname'] = limit_list_item_0_tscode_list
for table_name in limit_list_item_0.columns.values.tolist():
    limit_list_item_0_addname_dataframe[table_name] = limit_list_item_0[table_name]
limit_list_data_List = limit_list_data_List.append(limit_list_item_0_addname_dataframe)


limit_list_item_1_tscode_list = list() 
limit_list_item_1 = pro.limit_list(limit_type='D',end_date='20211130', fields='trade_date,ts_code,name,close,pct_chg,amp,fc_ratio,fl_ratio,fd_amount,first_time,last_time,open_times,strth,limit')
print("最近月涨跌停统计  limit_list_item_1 返回数据 row 行数 = "+str(limit_list_item_1.shape[0]))
for ts_code_sh in limit_list_item_1['ts_code']:
    stock_name = tscode_name_dict.get(ts_code_sh)
    if stock_name is None:
        stock_name = 'null'
    limit_list_item_1_tscode_list.append(stock_name)
limit_list_item_1_addname_dataframe=pd.DataFrame()
limit_list_item_1_addname_dataframe['cname'] = limit_list_item_1_tscode_list
for table_name in limit_list_item_1.columns.values.tolist():
    limit_list_item_1_addname_dataframe[table_name] = limit_list_item_1[table_name]
limit_list_data_List = limit_list_data_List.append(limit_list_item_1_addname_dataframe)


print("limit_list_data_List.__len__() = "+str(limit_list_data_List.__len__()))
limit_list_data_List.to_excel(limit_list_excel_writer,'最近月涨跌停统计',index=False)
limit_list_excel_writer.save()
