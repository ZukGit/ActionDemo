...
#  zukgit
# xianshougujiejin_zukgit_website  =   https://tushare.pro/document/2?doc_id=160
createexcel('限售股解禁.xlsx')
share_float_book = load_workbook('C:\\Users\\zhuzj5\\Desktop\\zbin\\J0_Data\\限售股解禁.xlsx')
share_float_excel_writer = pd.ExcelWriter('C:\\Users\\zhuzj5\\Desktop\\zbin\\J0_Data\\限售股解禁.xlsx', engine='openpyxl')
share_float_excel_writer.book = share_float_book
share_float_excel_writer.sheets = dict((ws.title, ws) for ws in share_float_book.worksheets)
share_float_data_List = pd.DataFrame()
share_float_item_0_tscode_list = list() 
share_float_item_0 = pro.share_float(start_date='20210101',end_date='20211231', fields='ts_code,ann_date,float_date,float_share,float_ratio,holder_name,share_type')
print("限售股解禁  share_float_item_0 返回数据 row 行数 = "+str(share_float_item_0.shape[0]))
for ts_code_sh in share_float_item_0['ts_code']:
    stock_name = tscode_name_dict.get(ts_code_sh)
    if stock_name is None:
        stock_name = 'null'
    share_float_item_0_tscode_list.append(stock_name)
share_float_item_0_addname_dataframe=pd.DataFrame()
share_float_item_0_addname_dataframe['cname'] = share_float_item_0_tscode_list
for table_name in share_float_item_0.columns.values.tolist():
    share_float_item_0_addname_dataframe[table_name] = share_float_item_0[table_name]
share_float_data_List = share_float_data_List.append(share_float_item_0_addname_dataframe)


print("share_float_data_List.__len__() = "+str(share_float_data_List.__len__()))
share_float_data_List.to_excel(share_float_excel_writer,'限售股解禁',index=False)
share_float_excel_writer.save()
