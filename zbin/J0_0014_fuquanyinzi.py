...
#  zukgit
# fuquanyinzi_zukgit_website  =   https://tushare.pro/document/2?doc_id=28
createexcel('复权因子.xlsx')
adj_factor_book = load_workbook('C:\\Users\\zhuzj5\\Desktop\\zbin\\J0_Data\\复权因子.xlsx')
adj_factor_excel_writer = pd.ExcelWriter('C:\\Users\\zhuzj5\\Desktop\\zbin\\J0_Data\\复权因子.xlsx', engine='openpyxl')
adj_factor_excel_writer.book = adj_factor_book
adj_factor_excel_writer.sheets = dict((ws.title, ws) for ws in adj_factor_book.worksheets)
adj_factor_data_List = pd.DataFrame()
adj_factor_item_0_tscode_list = list() 
adj_factor_item_0 = pro.adj_factor(ts_code='',trade_date='20211130', fields='ts_code,trade_date,adj_factor')
print("复权因子  adj_factor_item_0 返回数据 row 行数 = "+str(adj_factor_item_0.shape[0]))
for ts_code_sh in adj_factor_item_0['ts_code']:
    stock_name = tscode_name_dict.get(ts_code_sh)
    if stock_name is None:
        stock_name = 'null'
    adj_factor_item_0_tscode_list.append(stock_name)
adj_factor_item_0_addname_dataframe=pd.DataFrame()
adj_factor_item_0_addname_dataframe['cname'] = adj_factor_item_0_tscode_list
for table_name in adj_factor_item_0.columns.values.tolist():
    adj_factor_item_0_addname_dataframe[table_name] = adj_factor_item_0[table_name]
adj_factor_data_List = adj_factor_data_List.append(adj_factor_item_0_addname_dataframe)


print("adj_factor_data_List.__len__() = "+str(adj_factor_data_List.__len__()))
adj_factor_data_List.to_excel(adj_factor_excel_writer,'复权因子',index=False)
adj_factor_excel_writer.save()
