...
#  zukgit
# meirizhangdietingjiage_zukgit_website  =   https://tushare.pro/document/2?doc_id=183
createexcel('每日涨跌停价格.xlsx')
stk_limit_book = load_workbook('C:\\Users\\zhuzj5\\Desktop\\zbin\\J0_Data\\每日涨跌停价格.xlsx')
stk_limit_excel_writer = pd.ExcelWriter('C:\\Users\\zhuzj5\\Desktop\\zbin\\J0_Data\\每日涨跌停价格.xlsx', engine='openpyxl')
stk_limit_excel_writer.book = stk_limit_book
stk_limit_excel_writer.sheets = dict((ws.title, ws) for ws in stk_limit_book.worksheets)
stk_limit_data_List = pd.DataFrame()
stk_limit_item_0_tscode_list = list() 
stk_limit_item_0 = pro.stk_limit(trade_date='20211130', fields='ts_code,trade_date,pre_close,up_limit,down_limit')
print("每日涨跌停价格  stk_limit_item_0 返回数据 row 行数 = "+str(stk_limit_item_0.shape[0]))
for ts_code_sh in stk_limit_item_0['ts_code']:
    stock_name = tscode_name_dict.get(ts_code_sh)
    if stock_name is None:
        stock_name = 'null'
    stk_limit_item_0_tscode_list.append(stock_name)
stk_limit_item_0_addname_dataframe=pd.DataFrame()
stk_limit_item_0_addname_dataframe['cname'] = stk_limit_item_0_tscode_list
for table_name in stk_limit_item_0.columns.values.tolist():
    stk_limit_item_0_addname_dataframe[table_name] = stk_limit_item_0[table_name]
stk_limit_data_List = stk_limit_data_List.append(stk_limit_item_0_addname_dataframe)


print("stk_limit_data_List.__len__() = "+str(stk_limit_data_List.__len__()))
stk_limit_data_List.to_excel(stk_limit_excel_writer,'每日涨跌停价格',index=False)
stk_limit_excel_writer.save()
