...
#  zukgit
# gainiangufenlei_zukgit_website  =   https://tushare.pro/document/2?doc_id=125
createexcel('概念股分类.xlsx')
concept_book = load_workbook('C:\\Users\\zhuzj5\\Desktop\\zbin\\J0_Data\\概念股分类.xlsx')
concept_excel_writer = pd.ExcelWriter('C:\\Users\\zhuzj5\\Desktop\\zbin\\J0_Data\\概念股分类.xlsx', engine='openpyxl')
concept_excel_writer.book = concept_book
concept_excel_writer.sheets = dict((ws.title, ws) for ws in concept_book.worksheets)
concept_data_List = pd.DataFrame()
concept_item_0 = pro.concept(ts='ts', fields='code,name,src')
print(" 概念股分类 concept_item_0 返回数据 row 行数 = "+str(concept_item_0.shape[0]))
concept_data_List = concept_data_List.append(concept_item_0)


print("concept_data_List.__len__() = "+str(concept_data_List.__len__()))
concept_data_List.to_excel(concept_excel_writer,'概念股分类',index=False)
concept_excel_writer.save()
