...
#  zukgit
# ganggutongmeirichengjiaotongji_zukgit_website  =   https://tushare.pro/document/2?doc_id=196
createexcel('港股通每日成交统计.xlsx')
ggt_daily_book = load_workbook('C:\\Users\\zhuzj5\\Desktop\\zbin\\J0_Data\\港股通每日成交统计.xlsx')
ggt_daily_excel_writer = pd.ExcelWriter('C:\\Users\\zhuzj5\\Desktop\\zbin\\J0_Data\\港股通每日成交统计.xlsx', engine='openpyxl')
ggt_daily_excel_writer.book = ggt_daily_book
ggt_daily_excel_writer.sheets = dict((ws.title, ws) for ws in ggt_daily_book.worksheets)
ggt_daily_data_List = pd.DataFrame()
ggt_daily_item_0 = pro.ggt_daily(start_date='20210101',end_date='20211130', fields='trade_date,buy_amount,buy_volume,sell_amount,sell_volume')
print(" 港股通每日成交统计 ggt_daily_item_0 返回数据 row 行数 = "+str(ggt_daily_item_0.shape[0]))
ggt_daily_data_List = ggt_daily_data_List.append(ggt_daily_item_0)


print("ggt_daily_data_List.__len__() = "+str(ggt_daily_data_List.__len__()))
ggt_daily_data_List.to_excel(ggt_daily_excel_writer,'港股通每日成交统计',index=False)
ggt_daily_excel_writer.save()
