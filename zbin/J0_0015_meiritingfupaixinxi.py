...
#  zukgit
# meiritingfupaixinxi_zukgit_website  =   https://tushare.pro/document/2?doc_id=214
createexcel('每日停复牌信息.xlsx')
suspend_d_book = load_workbook('C:\\Users\\zhuzj5\\Desktop\\zbin\\J0_Data\\每日停复牌信息.xlsx')
suspend_d_excel_writer = pd.ExcelWriter('C:\\Users\\zhuzj5\\Desktop\\zbin\\J0_Data\\每日停复牌信息.xlsx', engine='openpyxl')
suspend_d_excel_writer.book = suspend_d_book
suspend_d_excel_writer.sheets = dict((ws.title, ws) for ws in suspend_d_book.worksheets)
suspend_d_data_List = pd.DataFrame()
suspend_d_item_0_tscode_list = list() 
suspend_d_item_0 = pro.suspend_d(suspend_type='S',start_date='20211123',end_date='20211207', fields='ts_code,trade_date,suspend_timing,suspend_type')
print("每日停复牌信息  suspend_d_item_0 返回数据 row 行数 = "+str(suspend_d_item_0.shape[0]))
for ts_code_sh in suspend_d_item_0['ts_code']:
    stock_name = tscode_name_dict.get(ts_code_sh)
    if stock_name is None:
        stock_name = 'null'
    suspend_d_item_0_tscode_list.append(stock_name)
suspend_d_item_0_addname_dataframe=pd.DataFrame()
suspend_d_item_0_addname_dataframe['cname'] = suspend_d_item_0_tscode_list
for table_name in suspend_d_item_0.columns.values.tolist():
    suspend_d_item_0_addname_dataframe[table_name] = suspend_d_item_0[table_name]
suspend_d_data_List = suspend_d_data_List.append(suspend_d_item_0_addname_dataframe)


suspend_d_item_1_tscode_list = list() 
suspend_d_item_1 = pro.suspend_d(suspend_type='R',start_date='20211123',end_date='20211207', fields='ts_code,trade_date,suspend_timing,suspend_type')
print("每日停复牌信息  suspend_d_item_1 返回数据 row 行数 = "+str(suspend_d_item_1.shape[0]))
for ts_code_sh in suspend_d_item_1['ts_code']:
    stock_name = tscode_name_dict.get(ts_code_sh)
    if stock_name is None:
        stock_name = 'null'
    suspend_d_item_1_tscode_list.append(stock_name)
suspend_d_item_1_addname_dataframe=pd.DataFrame()
suspend_d_item_1_addname_dataframe['cname'] = suspend_d_item_1_tscode_list
for table_name in suspend_d_item_1.columns.values.tolist():
    suspend_d_item_1_addname_dataframe[table_name] = suspend_d_item_1[table_name]
suspend_d_data_List = suspend_d_data_List.append(suspend_d_item_1_addname_dataframe)


print("suspend_d_data_List.__len__() = "+str(suspend_d_data_List.__len__()))
suspend_d_data_List.to_excel(suspend_d_excel_writer,'每日停复牌信息',index=False)
suspend_d_excel_writer.save()
