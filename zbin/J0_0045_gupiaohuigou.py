...
#  zukgit
# gupiaohuigou_zukgit_website  =   https://tushare.pro/document/2?doc_id=124
createexcel('股票回购.xlsx')
repurchase_book = load_workbook('C:\\Users\\zhuzj5\\Desktop\\zbin\\J0_Data\\股票回购.xlsx')
repurchase_excel_writer = pd.ExcelWriter('C:\\Users\\zhuzj5\\Desktop\\zbin\\J0_Data\\股票回购.xlsx', engine='openpyxl')
repurchase_excel_writer.book = repurchase_book
repurchase_excel_writer.sheets = dict((ws.title, ws) for ws in repurchase_book.worksheets)
repurchase_data_List = pd.DataFrame()
repurchase_item_0_tscode_list = list() 
repurchase_item_0 = pro.repurchase(start_date='20210101',end_date='20211231', fields='ts_code,ann_date,end_date,proc,exp_date,vol,amount,high_limit,low_limit')
print("股票回购  repurchase_item_0 返回数据 row 行数 = "+str(repurchase_item_0.shape[0]))
for ts_code_sh in repurchase_item_0['ts_code']:
    stock_name = tscode_name_dict.get(ts_code_sh)
    if stock_name is None:
        stock_name = 'null'
    repurchase_item_0_tscode_list.append(stock_name)
repurchase_item_0_addname_dataframe=pd.DataFrame()
repurchase_item_0_addname_dataframe['cname'] = repurchase_item_0_tscode_list
for table_name in repurchase_item_0.columns.values.tolist():
    repurchase_item_0_addname_dataframe[table_name] = repurchase_item_0[table_name]
repurchase_data_List = repurchase_data_List.append(repurchase_item_0_addname_dataframe)


print("repurchase_data_List.__len__() = "+str(repurchase_data_List.__len__()))
repurchase_data_List.to_excel(repurchase_excel_writer,'股票回购',index=False)
repurchase_excel_writer.save()
